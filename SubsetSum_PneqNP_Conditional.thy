...
The Cook--Levin execution model guarantees only that a machine
produces the correct final decision for a given input.
It places no constraints on how the machine internally processes,
compresses, reorganizes, or represents that input before reaching
its decision.

In particular, the execution semantics do not require that a machine's
computation explicitly expose or distinguish the left-hand and
right-hand candidate values arising from the subset-sum split
\<open>e_k (as, s)\<close>.

By contrast, the adversary-style lower bound developed in
\<open>SubsetSum_DecisionTree\<close> reasons precisely in terms of
distinguishing individual candidate values generated by that split.
The lower-bound argument depends on the necessity of making such
distinctions visible at the computational level.
...
underlying machine semantics alone.
